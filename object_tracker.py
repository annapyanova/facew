...
def main(_argv):
    ...
    try:
        vid = cv2.VideoCapture("filesrc location=/home/annapyanova/dev/facew/data/video/face.mp4 ! decodebin ! videoconvert ! appsink", cv2.CAP_GSTREAMER)
    ...
    while True:
        ...
        # if enable info flag then print details about each track
            ...
            cursor.execute(f"INSERT INTO facew VALUES('{int(track.track_id)}','{class_name}','{int(bbox[0])}', '{int(bbox[1])}', '{int(bbox[2])}', '{int(bbox[3])}');")
            conn.commit()
